  ...
  try {
    ...
    await base("Episodes")
      ...
      .eachPage(
        ...
        function done(err) {
          if (err) {
            console.error("Episode Query Error");
            console.error(err);
            return;
          }
        }
    ...
    // Get All Categories
    const categoryRecords = await base("iTunes Categories")
      .select({ maxRecords: 100 })
      .all();

    // Get All SubCategories
    const subcategoryRecords = await base("iTunes Subcategories")
      .select({ maxRecords: 100 })
      .all();
...
function episodeXML(episode) {
  ...
  <item>
    ...
    ${
      episode["<comments>"]
        ? `<comments>${encodeXml(episode["<comments>"])}</comments>`
        : ""
    }
    <pubDate>${encodeXml(
      new Date(episode["<pubDate>"]).toUTCString()
    )}</pubDate>
    <guid isPermaLink="false">${encodeXml(episode["<title>"])}</guid>
    <description><![CDATA[${encodeXml(
      episode["<description>"]
    )}]]></description>
    <content:encoded><![CDATA[${encodeXml(
      episode["<description>"]
    )}]]></content:encoded>           
    <enclosure url="${encodeXml(
      episode["<enclosure url>"]
    )}" length="${encodeXml(episode["<enclosure length>"])}" type="${encodeXml(
    ...
    <itunes:explicit>${encodeXml(
      episode["<itunes:explicit>"]
    )}</itunes:explicit>
    <itunes:block>${encodeXml(episode["<itunes:block>"])}</itunes:block>
    <itunes:duration>${encodeXml(
    ...
    <itunes:episodeType>${encodeXml(
      episode["<itunes:episodeType>"]
    )}</itunes:episodeType>
...
function categoryXML(category, subcategory) {
  return `<itunes:category text="${encodeXml(
    category
  )}"><itunes:category text="${encodeXml(subcategory)}"/></itunes:category>`;
}
...
module.exports.feed = async (event) => {
  ...
  const categoryItems = categories.map((category) => {
    ...
    const subcategory = subcategories.find(
      (record) => record.id === category.fields["<itunes:subcategory>"][0]
    );
    return categoryXML(
      category.fields["<itunes:category>"],
      subcategory.fields["<itunes:subcategory>"]
    );
  });
